...
import 'src/tree_with_mixin.dart';
import 'src/algebra.dart' as algebra;

final somePrettyTree = new PrettyTree("aaa",
    [new PrettyTree("bbbbb",
        [new PrettyTree("ccc", []),
         new PrettyTree("dd", [])]),
    new PrettyTree("eee", []),
    new PrettyTree("ffff",
...
void checkImplemMatchesModelForWidth(int width) {
  expect(
      somePrettyTree.pretty(implemFactory).render(width),
      equals(somePrettyTree.pretty(modelFactory).render(width)));
}
